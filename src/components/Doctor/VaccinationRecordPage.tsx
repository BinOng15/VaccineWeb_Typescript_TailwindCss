...
const VaccinationRecordPage: React.FC = () => {
  ...
  const [childMap, setChildMap] = useState<{ [key: number]: { childFullName: string; userFullName: string } }>({});
  ...
  const fetchVaccinationRecords = async (
    ...
    try {
      ...
      const newChildMap = childrenData.reduce((acc, { childId, childFullName, userId }) => {
        acc[childId] = {
          childFullName,
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      ...
      >
        {selectedRecord && (
          <div style={{ padding: 16 }}>
            <p><strong>Tên Trẻ em:</strong> {selectedChildName || "N/A"}</p>
            <p>
            ...
            <p><strong>Phản ứng:</strong> {selectedRecord.reaction || "N/A"}</p>
            <p><strong>Ghi chú:</strong> {selectedRecord.notes || "N/A"}</p>
            <p><strong>Hoạt động:</strong> {getIsActiveText(selectedRecord.isActive)}</p>
            <p>
            ...
            <p><strong>Người sửa đổi:</strong> {selectedRecord.modifiedBy || "N/A"}</p>
          </div>
        )}
      </Modal>
      ...
      >
        {selectedRecord && (
          <div style={{ padding: 16 }}>
            <p><strong>Phản ứng sau tiêm:</strong> {selectedRecord.reaction || "Không có phản ứng"}</p>
          </div>
        )}
      </Modal>
    </div>
