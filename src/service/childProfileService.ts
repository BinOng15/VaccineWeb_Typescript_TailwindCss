...
import { ChildProfileResponseDTO, PagedChildProfileResponse, CreateChildProfileDTO, UpdateChildProfileDTO } from "../models/ChildProfile";
...
const childProfileService = {
  ...
  getAllChildProfiles: async (): Promise<ChildProfileResponseDTO[]> => {
    try {
      const response: AxiosResponse = await axiosInstance.get("api/get-all");
  ...
  getChildProfileById: async (childId: number): Promise<ChildProfileResponseDTO> => {
    try {
  ...
  getChildProfilesPaged: async (pageNumber: number, pageSize: number): Promise<PagedChildProfileResponse> => {
    try {
  ...
  getChildProfileByName: async (keyword: string): Promise<ChildProfileResponseDTO[]> => {
    try {
      const response: AxiosResponse = await axiosInstance.get(`/get-by-name/${keyword}`);
      return response.data;
    } catch (error) {
      console.error(`Lỗi khi tìm kiếm hồ sơ trẻ em theo tên ${keyword}:`, error);
      throw error;
    }
  },

  // Tạo hồ sơ trẻ em mới
  createChildProfile: async (childData: CreateChildProfileDTO): Promise<ChildProfileResponseDTO> => {
    try {
      ...
      const response: AxiosResponse = await axiosInstance.post("/create-child-profile", formData, {
  ...
  updateChildProfile: async (id: number, childData: UpdateChildProfileDTO): Promise<ChildProfileResponseDTO> => {
    try {
      const formData = new FormData();
      formData.append("fullName", childData.fullName);
      if (childData.dateOfBirth) formData.append("dateOfBirth", childData.dateOfBirth);
      if (childData.gender) formData.append("gender", childData.gender.toString());
      if (childData.relationship) formData.append("relationship", childData.relationship.toString());
      if (childData.profilePicture) formData.append("profilePicture", childData.profilePicture);

      const response: AxiosResponse = await axiosInstance.put(`/update-child-profile/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      return response.data;
    } catch (error) {
